#!/usr/bin/env python3
"""Precompute HyenaDNA-tiny embeddings for the entire reference genome.

For each chromosome, the genome is divided into non-overlapping 14,000 bp core
regions. Each core is expanded by 1,000 bp of flanking sequence on either side
(filled with 'N' if past chromosome boundaries) to form a 16,000 bp window.
The window is tokenised character-by-character (1 bp = 1 token), passed through
HyenaDNA in a single forward pass, and the resulting (16000, 128) hidden states
are sliced to drop the flank tokens, reshaped to (280, 50, 128), and mean-pooled
along the 50-token axis to yield 280 embeddings of dimension 128 — one per
non-overlapping 50 bp window.

HDF5 layout
────────────
  /chr1   dataset  shape=(N_windows, 128)  dtype=float16
  /chr2   dataset  shape=(N_windows, 128)  dtype=float16
  ...
  attrs:
    ...
    core_bp         — int, core region per chunk (14000)
    seq_bp          — int, full input sequence per chunk (16000)
    embed_dim       — int, 128
...
───────
  ...
  with h5py.File("embeddings.h5", "r") as f:
      vec = f["chr21"][window_idx]    # (128,) float16
"""
...
HYENADNA_MODEL_ID = "LongSafari/hyenadna-tiny-16k-seqlen-d128-hf"
...
CORE_BP = 14_000  # non-overlapping core per forward pass
FLANK_BP = 1_000  # flank context on each side
SEQ_BP = CORE_BP + 2 * FLANK_BP  # 16_000 — total input length
...
def fetch_chunk_sequence(
    ...
    """Return a 16_000-bp uppercase string for chunk `chunk_idx`.
...
def embed_chunk(
    ...
    """Run one chunk through HyenaDNA and return (280, 128) float16 embeddings.

    Steps:
      1. Tokenise the 16_000-char string (character tokenizer → 16_000 tokens).
      2. Forward pass under bfloat16 autocast → (1, 16_000, 128) hidden states.
      3. Drop the flanks: keep positions [FLANK_BP : FLANK_BP + CORE_BP].
      4. Reshape to (1, 280, 50, 128) and mean over the 50-token axis.
    """
    encoded = tokenizer(
        seq,
        return_tensors="pt",
    ...
    hidden = outputs.last_hidden_state  # (1, 16000, 128)

    core = hidden[:, FLANK_BP : FLANK_BP + CORE_BP, :]  # (1, 14000, 128)
    pooled = core.reshape(1, WINDOWS_PER_CHUNK, WINDOW_BP, EMBED_DIM).mean(dim=2)
    pooled = pooled.squeeze(0).to(torch.float16).cpu().numpy()  # (280, 128)
    return pooled
...
def precompute(
    ...
    for chrom in chroms_to_process:
        ...
        for k in range(n_chunks):
            ...
            pooled = embed_chunk(seq, model, tokenizer, device)  # (280, 128)
